...
(function(window, undefined) {
	...
	function createKeyboard() {
		var key="";
		var html = '<div id="jim-ipad-kb" onselectstart="return false;">';
	...
	function bindKeyboard() {
		jQuery("#jim-ipad-kb").on("mouseup", function(event, data) {
		...
		jQuery("#jim-ipad-kb").on("mousedown", function(event, data) {
		...
		jQuery("#jim-ipad-kb").on("dblclick", function(event, data) {
		...
		jQuery("#jim-ipad-kb").on("mouseleave", function(event, data) {
		  if(pressedTarget) {
			removeEffects(pressedTarget);
		    pressedTarget=null;
		  }
	...
	function applyNewValue(oldValue, newKey, $target) {
		...
		switch(newKey) {
			...
			case "return":
			case "exit":
				jimMobile.slideKeyboard(jQuery("#jim-ipad-kb"), false);
				deactivateSpecialKeys();
	...
	function createDropDown() {
		var html = '<div id="jim-ipad-dd" onselectstart="return false;"><div id="jim-ipad-dd_options"><div class="dd_options"></div></div></div>';
		jQuery("#jim-container").append(html);
		jQuery('#jim-ipad-dd_options .dd_options').draggable({ 
			axis: "y",
	...
	function bindDropDown() {
		jQuery("#jim-ipad-dd").on("mouseup", function(event, data) {
			if(dragStart) {
				dragStart = false;
			}
				...
				if(key) {
					...
					case"jim-ipad-dd_options":
						...
						case "options":
							...
							jQuery("#jim-ipad-dd").fadeOut(300);
		...
		jQuery("#jim-ipad-dd").on("mousedown", function(event, data) {
				...
				switch(key) {
					...
					case "options":
						...
						if($target[0]===$currentPressed[0]) {
							jQuery("#jim-ipad-dd_options .dd_options .tick").removeClass("tick").addClass("tock");
							jQuery("#jim-ipad-dd_options .dd_options .pressed").removeClass("pressed");
							$target.parent().removeClass("pressed").addClass("pressed");
						}
						else {
							jQuery("#jim-ipad-dd_options .dd_options .tick").removeClass("tick").addClass("tock").hide();
							jQuery("#jim-ipad-dd_options .dd_options .pressed").removeClass("pressed");
							$target.parent().removeClass("pressed").addClass("pressed");			
						}
						break;
		...
		jQuery("#jim-ipad-dd").on("dragstart", function(event, data) {
			jQuery(".dd_options .pressed").removeClass("pressed");
			jQuery(".dd_options .tock").removeClass("tock").addClass("tick").show();
			startDDPos = parseInt(jQuery(".dd_options").css("top"), 10);
			dragStart = true;
		});
		
		jQuery("#jim-ipad-dd").on("drag", function(event, data) {
			if(dragStart) {
				var offset = jQuery("#jim-ipad-dd_options").offset(),
				dd_width = parseInt(jQuery("#jim-ipad-dd_options").css('width')),
				dd_height = parseInt(jQuery("#jim-ipad-dd_options").css('height')),
				...
				if(posY<offset.top) {
					event.preventDefault();
					jQuery(".dd_options").animate({ "top" : (-(44*(ddSize-4))) +"px"});
					dragStart=false;
				}
				else if(posY>offset.top+(dd_height/zoom)) {
					event.preventDefault();
					jQuery(".dd_options").animate({ "top" : "0px"});
					dragStart=false;
				}
			}
		...
		switch(type) {
			...
			case itemType.nativedropdown:
				...
				if(selectedPos>=0 && selectedPos>3) {
					jQuery(".dd_options").css("top", -(selectedPos*44)+44 + "px");
				}
				break;
		}
	}
	...
	function setDropDrownValue($target) {
		...
		switch(type) {
			...
			case itemType.nativedropdown:
				...
				var value = jQuery(".dd_options .line_options.selected .options").text();
	...
	function createDate() {
		var html = "";
		if($.browser.msie)
			html = '<div id="jim-ipad-da"><div id="jim-ipad-da_controls"><div class="da_controls-clear">Clear</div></div><div><div class="jim-ipad-da_mask"></div><div id="jim-ipad-da_options"><div class="da_months"></div><div class="da_days"></div><div class="da_years"></div></div><div id="jim-ipad-da_options_big"><div class="da_months"></div><div class="da_days"></div><div class="da_years"></div></div></div></div>';
		else html = '<div id="jim-ipad-da"><div id="jim-ipad-da_controls"><div class="da_controls-clear">Clear</div></div><div><div id="jim-ipad-da_options"><div class="da_months"></div><div class="da_days"></div><div class="da_years"></div></div><div id="jim-ipad-da_options_big"><div class="da_months"></div><div class="da_days"></div><div class="da_years"></div></div><div class="jim-ipad-da_mask"></div></div></div>';
		jQuery("#jim-container").append(html);
	...
	function bindDateControls() {
		jQuery("#jim-ipad-da").on("mouseup", function(event, data) {
			var realTarget = event.target || event.srcElement;
		...
		jQuery("#jim-ipad-da").on("mousedown", function(event, data) {
			var realTarget = event.target || event.srcElement;
			if (realTarget.tagName === "DIV" && !dragStart) {
				var key = realTarget.className;
				if(key.indexOf(" ")>0)
	...
	function createTime() {
		var html = "";
		if($.browser.msie)
			html = '<div id="jim-ipad-ti"><div id="jim-ipad-ti_controls"><div class="ti_controls-clear">Clear</div></div><div><div class="jim-ipad-ti_mask"></div><div id="jim-ipad-ti_options"><div class="ti_hours"></div><div class="ti_minutes"></div><div class="ti_periods"></div></div><div id="jim-ipad-ti_options_big"><div class="ti_hours"></div><div class="ti_minutes"></div><div class="ti_periods"></div></div></div></div>';
		else html = '<div id="jim-ipad-ti"><div id="jim-ipad-ti_controls"><div class="ti_controls-clear">Clear</div></div><div><div id="jim-ipad-ti_options"><div class="ti_hours"></div><div class="ti_minutes"></div><div class="ti_periods"></div></div><div id="jim-ipad-ti_options_big"><div class="ti_hours"></div><div class="ti_minutes"></div><div class="ti_periods"></div></div><div class="jim-ipad-ti_mask"></div></div></div>';
		jQuery("#jim-container").append(html);
	...
	function bindTimeControls() {
		jQuery("#jim-ipad-ti").on("mouseup", function(event, data) {
			if (event.target.tagName === "DIV" && !dragStart) {
				var key = event.target.className;
				if(key) {
					if(key.indexOf(" ")>0)
		...
		jQuery("#jim-ipad-ti").on("mousedown", function(event, data) {
			if (event.target.tagName === "DIV" && !dragStart) {
				var key = event.target.className;
				if(key.indexOf(" ")>0)
					key=key.substring(0, key.indexOf(" "));
	...
	function bindDateTimeControls() {
		jQuery("#jim-ipad-dt").on("mouseup", function(event, data) {
			if (event.target.tagName === "DIV" && !dragStart) {
				var key = event.target.className;
				if(key) {
					if(key.indexOf(" ")>0)
		...
		jQuery("#jim-ipad-dt").on("mousedown", function(event, data) {
			if (event.target.tagName === "DIV" && !dragStart) {
				var key = event.target.className;
				if(key.indexOf(" ")>0)
					key=key.substring(0, key.indexOf(" "));
	...
	function checkExternalClick(event) {
		...
		if(input && ( (($target.closest(".text")[0]!==input[0]) && ($target.closest(".text").length===0 && $target.closest(".password").length===0 && $target.closest(".textarea").length===0)) || 
				...
				&& $target[0].id != $("#jim-ipad-kb") && !$target.closest("#jim-ipad-kb").length && $("#jim-ipad-kb").css("display")!=="none" && !$("#jim-ipad-kb:animated").length) {
			jimMobile.slideKeyboard(jQuery("#jim-ipad-kb"), false);
		...
		if(input && ($target.closest(".dropdown, .nativedropdown")[0]!==input[0]) && !$target.is(".dropdown, .nativedropdown") && $target[0].id != $("#jim-ipad-dd") && !$target.closest("#jim-ipad-dd").length && $("#jim-ipad-dd").css("display")!=="none") {
			jQuery("#jim-ipad-dd").fadeOut(300);
			jQuery("#jim-ipad-mask").hide();
		...
		if(input && ($target.closest(".date")[0]!==input[0]) && $target.closest(".date").length===0 && $target[0].id != $("#jim-ipad-da") && !$target.closest("#jim-ipad-da").length && $("#jim-ipad-da").css("display")!=="none") {
			jQuery("#jim-ipad-da").fadeOut(300);
			jQuery("#jim-ipad-mask").hide();
		...
		if(input && ($target.closest(".time")[0]!==input[0]) && $target.closest(".time").length===0 && $target[0].id != $("#jim-ipad-ti") && !$target.closest("#jim-ipad-ti").length && $("#jim-ipad-ti").css("display")!=="none") {
			jQuery("#jim-ipad-ti").fadeOut(300);
			jQuery("#jim-ipad-mask").hide();
			var value = input.find("input").val();
			if(initialInputValue!==value) {
				input.closest(".firer").trigger("change");
		...
		if(input && ($target.closest(".datetime")[0]!==input[0]) && $target.closest(".datetime").length===0 && $target[0].id != $("#jim-ipad-dt") && !$target.closest("#jim-ipad-dt").length && $("#jim-ipad-dt").css("display")!=="none") {
			jQuery("#jim-ipad-dt").fadeOut(300);
			jQuery("#jim-ipad-mask").hide();
			var value = input.find("input").val();
			if(initialInputValue!==value) {
				input.closest(".firer").trigger("change");
	...
	$("#jim-container").on("mousewheel DOMMouseScroll", checkExternalClick);
	...
	jQuery.extend(jimMobile, {
		...
		"loadKeyboard": function() {
			...
			jQuery("#simulation").delegate(".text:not(.number, .email, .inputurl) input:not([readonly]), .password input:not([readonly]), textarea:not([readonly])", "click, focusin", function(event, data) {
				if(!jQuery("#jim-ipad-kb").css("display") || jQuery("#jim-ipad-kb").css("display") === "none") {
					jQuery("#jim-ipad-mask").hide();
					jimMobile.slideKeyboard(jQuery("#jim-ipad-kb"), true);
				}
				lastKeyboard = "#letters";
		...
		"unloadKeyboard": function() {
			if(jQuery("#jim-ipad-kb").length>0) {
				jQuery("#simulation").undelegate(".text", "click");
				if(jQuery("#jim-ipad-kb").css("display") !== "none")
					jimMobile.slideKeyboard(jQuery("#jim-ipad-kb"), false);
				jQuery("#jim-ipad-kb").remove();
			}
		},
		"loadDropDown": function() {
			...
			jQuery("#simulation").delegate(".dropdown:not([readonly]), .nativedropdown:not([readonly])", "click", function(event, data) {
				...
				var position = jimMobile.getCurrentPosition(input),
				width = parseInt(input.css("width"))/(1/jimUtil.getScale()),
				height = parseInt(input.css("height"))/(1/jimUtil.getScale());
				jQuery("#jim-ipad-dd").css("top", position.y/(1/jimUtil.getScale()) + height + "px");
				jQuery("#jim-ipad-dd").css("left", position.x/(1/jimUtil.getScale()) + (width/2) - (parseInt(jQuery("#jim-ipad-dd").css("width"))/2) + "px");
				if(jQuery("#jim-ipad-dd").css("display") === "none") {
					jQuery("#jim-ipad-mask").show();
					jQuery("#jim-ipad-dd").show();
				}
				//input.closest(".firer").trigger("focusin");
			});
		},
		"unloadDropDown": function() {
			if(jQuery("#jim-ipad-dd").length>0) {
				jQuery("#simulation").undelegate(".dropdown, .nativedropdown", "click");
				if(jQuery("#jim-ipad-dd").css("display") !== "none")
					jQuery("#jim-ipad-dd").hide();
				jQuery("#jim-ipad-dd").remove();
		...
		"loadDate": function() {
			...
			jQuery("#simulation").delegate(".date input:not([readonly])", "click, focusin", function(event, data) {
				...
				width = parseInt(input.css("width"))/(1/jimUtil.getScale()),
				height = parseInt(input.css("height"))/(1/jimUtil.getScale());
				jQuery("#jim-ipad-da").css("top", position.y/(1/jimUtil.getScale()) + height + "px");
				jQuery("#jim-ipad-da").css("left", position.x/(1/jimUtil.getScale()) + (width/2) - (parseInt(jQuery("#jim-ipad-da").css("width"))/2) + "px");
				fillDate();
				if(jQuery("#jim-ipad-da").css("display") === "none") {
					jQuery("#jim-ipad-mask").show();
					jQuery("#jim-ipad-da").show();
				}
				//input.closest(".firer").trigger("focusin");
			});
		},
		"unloadDate": function() {
			if(jQuery("#jim-ipad-da").length>0) {
				jQuery("#simulation").undelegate(".date", "click");
				if(jQuery("#jim-ipad-da").css("display") !== "none")
					jQuery("#jim-ipad-da").hide();
				jQuery("#jim-ipad-da").remove();
				jQuery("#jim-ipad-mask").remove();
			}
		},
		"loadTime": function() {
			...
			jQuery("#simulation").delegate(".time input:not([readonly])", "click, focusin", function(event, data) {
				...
				width = parseInt(input.css("width"))/(1/jimUtil.getScale()),
				height = parseInt(input.css("height"))/(1/jimUtil.getScale());
				jQuery("#jim-ipad-ti").css("top", position.y/(1/jimUtil.getScale()) + height + "px");
				jQuery("#jim-ipad-ti").css("left", position.x/(1/jimUtil.getScale()) + (width/2) - (parseInt(jQuery("#jim-ipad-ti").css("width"))/2) + "px");
				if(jQuery("#jim-ipad-ti").css("display") === "none") {
					jQuery("#jim-ipad-mask").show();
					jQuery("#jim-ipad-ti").show();
				}
				//input.closest(".firer").trigger("focusin");
			});
		},
		"unloadTime": function() {
			if(jQuery("#jim-ipad-ti").length>0) {
				jQuery("#simulation").undelegate(".time", "click");
				if(jQuery("#jim-ipad-ti").css("display") !== "none")
					jQuery("#jim-ipad-ti").hide();
				jQuery("#jim-ipad-ti").remove();
				jQuery("#jim-ipad-mask").remove();
			}
		},
		"loadDateTime": function() {
			...
			jQuery("#simulation").delegate(".datetime input:not([readonly])", "click, focusin", function(event, data) {
				...
				width = parseInt(input.css("width"))/(1/jimUtil.getScale()),
				height = parseInt(input.css("height"))/(1/jimUtil.getScale());
				jQuery("#jim-ipad-dt").css("top", position.y/(1/jimUtil.getScale()) + height + "px");
				jQuery("#jim-ipad-dt").css("left", position.x/(1/jimUtil.getScale()) + (width/2) - (parseInt(jQuery("#jim-ipad-dt").css("width"))/2) + "px");
				if(jQuery("#jim-ipad-dt").css("display") === "none") {
					jQuery("#jim-ipad-mask").show();
					jQuery("#jim-ipad-dt").show();
				}
			});
		},
		"unloadDateTime": function() {
			if(jQuery("#jim-ipad-dt").length>0) {
				jQuery("#simulation").undelegate(".datetime", "click");
				if(jQuery("#jim-ipad-dt").css("display") !== "none")
					jQuery("#jim-ipad-dt").hide();
				jQuery("#jim-ipad-dt").remove();
				jQuery("#jim-ipad-mask").remove();
			}
		},
		"resetWidgets": function() {
			...
			if(jQuery(input).closest(".number").find("input").length > 0) {
				jQuery("#jim-ipad-kb #letters").css('display', 'none');
				jQuery("#jim-ipad-kb #numbers").css('display', 'block');
				jQuery("#jim-ipad-kb #signs").css('display', 'none');
				jQuery("#jim-ipad-kb #emailurl").css('display', 'none');
			}
			else if(jQuery(input).closest(".inputurl").find("input").length > 0) {
				jQuery("#jim-ipad-kb #letters").css('display', 'none');
				jQuery("#jim-ipad-kb #numbers").css('display', 'none');
				jQuery("#jim-ipad-kb #signs").css('display', 'none');
				jQuery("#jim-ipad-kb #emailurl").css('display', 'block');
			}
				...
				jQuery("#jim-ipad-kb #letters").css('display', 'block');
				jQuery("#jim-ipad-kb #numbers").css('display', 'none');
				jQuery("#jim-ipad-kb #signs").css('display', 'none');
				jQuery("#jim-ipad-kb #emailurl").css('display', 'none');
			}
			deactivateSpecialKeys();
			//dropdown
			jQuery("#jim-ipad-dd .dd_options").css("top", "");
			//date
			jQuery("#jim-ipad-da .da_months").css("top", "");
			jQuery("#jim-ipad-da .da_days").css("top", "");
			jQuery("#jim-ipad-da .da_years").css("top", "");
			//time
			jQuery("#jim-ipad-ti .ti_hours").css("top", "");
			jQuery("#jim-ipad-ti .ti_minutes").css("top", "");
			jQuery("#jim-ipad-ti .ti_periods").css("top", "");
			//datetime
			jQuery("#jim-ipad-dt .dt_date").css("top", "");
			jQuery("#jim-ipad-dt .dt_hours").css("top", "");
			jQuery("#jim-ipad-dt .dt_minutes").css("top", "");
			jQuery("#jim-ipad-dt .dt_periods").css("top", "");
		},
		"hideWidgets": function() {
			//keyboard
			jQuery("#jim-ipad-kb").css('display', 'none');
			//dropdown
			jQuery("#jim-ipad-dd").css('display', 'none');
			//date
			jQuery("#jim-ipad-da").css('display', 'none');
			//time
			jQuery("#jim-ipad-ti").css('display', 'none');
			//datetime
			jQuery("#jim-ipad-dt").css('display', 'none');
		},
